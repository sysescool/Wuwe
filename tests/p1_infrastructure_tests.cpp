...
wuwe::llm_response failure(wuwe::agent::llm_error_code code) {
  return { .content = "scripted failure", .error_code = wuwe::agent::make_error_code(code) };
}
...
void scripted_client_and_fallback() {
  ...
  require(!response.error_code && response.content == "recovered" &&
            response.metadata.at("backend_id") == "fallback" && primary->remaining() == 0 &&
...
void scripted_predicates_are_reentrant_and_deterministic() {
  ...
  client.push({
    .matches = [&client](const wuwe::llm_request&) { return client.remaining() == 1; },
...
void resilience_recovers_backend_exceptions_and_preserves_callback_failures() {
  ...
  llm::resilient_llm_client retrying({ { .id = "throwing", .client = throwing } },
    { .retry = {
        .max_retries = 1, .initial_backoff = 0ms, .max_backoff = 0ms, .jitter_ratio = 0.0 } });
  const auto recovered = retrying.complete({});
  require(recovered.content == "recovered from exception" && throwing->calls == 2,
  ...
  llm::resilient_llm_client protected_callbacks({ { .id = "streaming", .client = streaming } },
    { .retry = {
        .max_retries = 1, .initial_backoff = 0ms, .max_backoff = 0ms, .jitter_ratio = 0.0 } });
...
void resilience_aggregates_usage_across_retries_and_fallbacks() {
  ...
  const auto response = client.complete({});
  require(response.content == "fallback" && response.usage.prompt_tokens == 15 &&
            response.usage.completion_tokens == 6 && response.usage.total_tokens == 21 &&
            response.usage.cached_prompt_tokens == 4 && response.usage.reasoning_tokens == 3,
...
void resilience_boundaries() {
  {
    ...
    llm::resilient_llm_client client({ { .id = "retry", .client = backend } },
      { .retry = { .max_retries = 1,
          .initial_backoff = 0ms,
          .max_backoff = 0ms,
          .max_server_delay = 0ms,
          .jitter_ratio = 0.0 } });
  ...
  }
  {
    ...
    llm::resilient_llm_client client({ { .id = "circuit", .client = backend } },
  ...
  }
  {
    ...
    llm::resilient_llm_client client({ { .id = "limited", .client = backend } },
    ...
    require(
      client.complete({}).error_code == wuwe::agent::llm_error_code::rate_limit_wait_exceeded &&
        backend->remaining() == 1,
  ...
  }
  {
    ...
    llm::resilient_llm_client client({ { .id = "stream", .client = backend } },
      { .retry = {
          .max_retries = 1, .initial_backoff = 0ms, .max_backoff = 0ms, .jitter_ratio = 0.0 } });
    int errors = 0;
    ...
    require(response.error_code == wuwe::agent::llm_error_code::timeout && errors == 1 &&
              backend->remaining() == 1,
  ...
  }
  {
    ...
    llm::resilient_llm_client client({ { .id = "throwing-jitter", .client = backend } },
      { .retry = { .max_retries = 1,
          .initial_backoff = 0ms,
          .max_backoff = 0ms,
          .jitter_ratio = 0.5 },
        .random_unit = []() -> double { throw std::runtime_error("random source unavailable"); } });
  ...
  }
  {
    ...
    llm::resilient_llm_client client({ { .id = "nan-jitter", .client = backend } },
      { .retry = { .max_retries = 1,
          .initial_backoff = 0ms,
          .max_backoff = 0ms,
          .jitter_ratio = 0.5 },
        .random_unit = [] { return (std::numeric_limits<double>::quiet_NaN)(); } });
...
void detailed_usage_and_cost() {
  ...
  const auto saturated =
    llm::calculate_llm_cost({ .prompt_tokens = (std::numeric_limits<int>::max)(),
                              .total_tokens = (std::numeric_limits<int>::max)() },
      { .input_per_million_tokens_usd = (std::numeric_limits<double>::max)() });
  require(saturated && std::isfinite(saturated->input_usd) && std::isfinite(saturated->total_usd),
  ...
  require(accumulated.prompt_tokens == 9 && accumulated.completion_tokens == 14 &&
...
void context_budget_accounting_saturates() {
  ...
  llm::context_budget_manager manager(std::make_shared<maximum_estimator>());
  ...
  require(
    !result && result.report.before.input_total() == (std::numeric_limits<std::size_t>::max)(),
    "context accounting must saturate instead of wrapping a custom estimator");
}

void observability_and_replay() {
  obs::agent_event event { .module = "runtime",
    .name = "tool_completed",
    .trace_id = "trace-1",
    .subject_id = "run-1",
    .run_id = "run-1",
  ...
  require(stats.published == 1 && stats.delivered == 1 && stats.failures == 0 &&
            memory->events().size() == 1,
  ...
  const auto transitioned =
    run.transition(record.id, record.revision, runtime::agent_run_status::running, "run_started");
...
void security_evaluation() {
  ...
  const auto unsafe = wuwe::agent::evaluation::make_security_evaluation_case("unsafe",
    wuwe::agent::evaluation::security_scenario::cross_tenant_access,
...
void mcp_protocol_negotiation() {
  ...
  const auto legacy = server.handle_message(nlohmann::json {
    ...
    { "params", { { "protocolVersion", "2024-11-05" } } },
                                              ...
                                              .dump());
  require(legacy && nlohmann::json::parse(*legacy)["result"]["protocolVersion"] == "2024-11-05" &&
  ...
  require(response.contains("error") && response["error"]["data"]["supportedVersions"].size() == 2,
...
void sqlite_migrations() {
  ...
  sqlite3_exec(db,
    "CREATE TABLE agent_runs (id TEXT PRIMARY KEY, revision INTEGER NOT NULL, document_json TEXT "
    "NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(db,
    "CREATE TABLE agent_run_events (run_id TEXT NOT NULL, sequence INTEGER NOT NULL, document_json "
    "TEXT NOT NULL, PRIMARY KEY(run_id, sequence));",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_close(db);
  ...
  sqlite3_exec(db,
    "CREATE TABLE agent_runtime_schema (component TEXT PRIMARY KEY, version INTEGER NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(
    db, "INSERT INTO agent_runtime_schema VALUES ('run_store', 99);", nullptr, nullptr, nullptr);
  sqlite3_close(db);
  bool rejected = false;
  try {
    runtime::sqlite_agent_run_store store(future_path);
  }
  ...
  const auto memory_path = base.string() + "-memory.db";
  {
    ...
    require(
      store.schema_version() == wuwe::agent::memory::sqlite_memory_store::latest_schema_version &&
  ...
  require(sqlite3_open(memory_path.c_str(), &db) == SQLITE_OK, "open memory migration fixture");
  sqlite3_exec(db,
  ...
  sqlite3_close(db);
  {
    ...
    require(
      store.schema_version() == wuwe::agent::memory::sqlite_memory_store::latest_schema_version &&
        restored && restored->content == "preserved",
      "unversioned memory schema should migrate without losing records");
  }
  require(sqlite3_open(memory_path.c_str(), &db) == SQLITE_OK, "open memory v1 migration fixture");
  ...
  require(sqlite3_open(memory_path.c_str(), &db) == SQLITE_OK, "open future memory fixture");
  sqlite3_exec(db,
  ...
  require(
    sqlite3_open(knowledge_path.c_str(), &db) == SQLITE_OK, "open knowledge migration fixture");
  sqlite3_exec(db,
  ...
  sqlite3_close(db);
  {
    ...
    require(index.schema_version() ==
                wuwe::agent::knowledge::sqlite_knowledge_index::latest_schema_version &&
              index.search({ .text = "preserved", .limit = 1 }, { 1.0F, 0.0F }).size() == 1,
      "unversioned knowledge schema should migrate without losing entries");
  }
  require(sqlite3_open(knowledge_path.c_str(), &db) == SQLITE_OK, "open future knowledge fixture");
  sqlite3_exec(db,
    "UPDATE wuwe_storage_schema SET version = 99 "
  ...
  require(sqlite3_open(malformed_run_path.c_str(), &db) == SQLITE_OK, "open malformed run fixture");
  sqlite3_exec(db,
    "CREATE TABLE agent_runtime_schema (component TEXT PRIMARY KEY, version INTEGER NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(
    db, "INSERT INTO agent_runtime_schema VALUES ('run_store', 1);", nullptr, nullptr, nullptr);
  sqlite3_exec(db,
    "CREATE TABLE agent_runs (id TEXT PRIMARY KEY, revision INTEGER NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(db,
  ...
  rejected = false;
  try {
    runtime::sqlite_agent_run_store store(malformed_run_path);
  }
  catch (const std::runtime_error&) {
    rejected = true;
  }
  ...
  require(
    sqlite3_open(malformed_memory_path.c_str(), &db) == SQLITE_OK, "open malformed memory fixture");
  sqlite3_exec(db,
    "CREATE TABLE wuwe_storage_schema (component TEXT PRIMARY KEY, version INTEGER NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(
    db, "INSERT INTO wuwe_storage_schema VALUES ('memory_store', 2);", nullptr, nullptr, nullptr);
  sqlite3_exec(db, "CREATE TABLE memory_records (id TEXT PRIMARY KEY);", nullptr, nullptr, nullptr);
  sqlite3_exec(db,
    "CREATE TABLE wuwe_memory_sequence (name TEXT PRIMARY KEY, last_value INTEGER NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_close(db);
  rejected = false;
  try {
    wuwe::agent::memory::sqlite_memory_store store(malformed_memory_path);
  }
  catch (const std::runtime_error&) {
    rejected = true;
  }
  ...
  sqlite3_exec(db,
    ...
    nullptr);
  sqlite3_exec(db,
    "INSERT INTO wuwe_storage_schema VALUES ('knowledge_index', 1);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_exec(db,
    "CREATE TABLE knowledge_index (chunk_id TEXT PRIMARY KEY, document_id TEXT NOT NULL);",
    nullptr,
    nullptr,
    nullptr);
  sqlite3_close(db);
  rejected = false;
  try {
    wuwe::agent::knowledge::sqlite_knowledge_index index(malformed_knowledge_path);
  }
  catch (const std::runtime_error&) {
    rejected = true;
  }
  require(rejected, "knowledge index should reject a versioned table with an incompatible shape");
...
void sqlite_unicode_paths() {
  ...
  }
  {
    ...
    index.upsert(
      ...
      { 1.0F, 0.0F });
    require(index.search({ .text = "knowledge", .limit = 1 }, { 1.0F, 0.0F }).size() == 1,
...
int main() {
  try {
    ...
    run("resilience usage aggregation", resilience_aggregates_usage_across_retries_and_fallbacks);
