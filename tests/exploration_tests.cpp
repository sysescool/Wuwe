...
exploration::exploration_runner make_runner(
  ...
  return exploration::exploration_runner({
    ...
    .execute = [&](const exploration::experiment& experiment,
                   const exploration::exploration_context&) {
      ++executions;
    ...
    .review = [confidence](const exploration::exploration_request&,
...
void approval_enables_explicit_effectful_experiments() {
  ...
  require(effectful.approval &&
      effectful.approval->kind == approval::approval_decision_kind::approved,
    "effectful experiment retains its approval evidence");
  require(std::any_of(
      effectful.specification.capabilities.begin(),
...
void low_confidence_reviews_remain_inconclusive() {
  ...
  require(result && result.supported_count == 0 && result.inconclusive_count == 1 &&
      result.record.hypotheses.front().verdict ==
        exploration::hypothesis_verdict::inconclusive,
...
void bounds_uncooperative_experiments() {
  ...
  require(!result &&
      result.record.stop_reason == exploration::exploration_stop_reason::timed_out &&
      result.record.detached_count == 1 &&
      result.record.hypotheses.front().experiments.front().detached,
...
void reports_total_design_failure() {
  exploration::exploration_runner runner({
    ...
    .design = [](const exploration::exploration_request&,
                 const exploration::hypothesis&,
                 const exploration::exploration_context&)
                 -> std::vector<exploration::experiment> {
  ...
  require(!result && result.record.stop_reason ==
      exploration::exploration_stop_reason::design_failed &&
...
void telemetry_failures_follow_the_common_policy() {
  ...
  exploration::exploration_runner isolated(options(
    throwing,
    wuwe::agent::observability::telemetry_failure_mode::ignore));
  ...
  exploration::exploration_runner strict(options(
    throwing,
