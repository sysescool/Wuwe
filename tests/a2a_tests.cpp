...
public:
  a2a::rpc_result invoke(std::string, nlohmann::json, std::stop_token) override {
...
public:
  a2a::result<a2a::task> send(const a2a::send_message_params& params, std::stop_token) override {
    a2a::task value {
...
void codecs_and_in_process_protocol_work() {
  ...
  require(
    static_cast<bool>(client.discover()), "in-process transport exposes Agent Card discovery");
  ...
  require(static_cast<bool>(client.get({ .id = "task-1" })), "tasks/get retrieves task state");
...
public:
  wuwe::http_response send(const wuwe::http_request& request) override {
    ...
    if (request.method == "GET") {
      if (discovery_stop_source)
        discovery_stop_source->request_stop();
    ...
    auto body = nlohmann::json {
      { "jsonrpc", "2.0" },
      { "id",
        wrong_response_id ? nlohmann::json(rpc.at("id").get<std::uint64_t>() + 1) : rpc.at("id") },
      { "result",
        a2a::to_json(a2a::task {
          .id = "task-http",
          .status = { .state = a2a::task_state::completed },
        }) },
    };
...
void http_client_and_service_adapters_work() {
  ...
  require(http->requests.front().url == "https://agents.example/.well-known/agent-card.json",
  ...
  require(
    rpc.at("method") == "message/send" && rpc.at("params").at("message").at("kind") == "message",
  ...
  require(!mismatched && mismatched.failure->code == a2a::error_code::invalid_agent_response,
  ...
  require(
    !cancelled_discovery && cancelled_discovery.failure->code == a2a::error_code::transport_error,
...
void service_rejects_unsupported_advertised_capabilities() {
  ...
  require(rejected, "synchronous services cannot advertise unsupported streaming capabilities");
}
...
void jsonrpc_notifications_and_invalid_requests_are_handled() {
  ...
  require(malformed.at("error").at("code") == static_cast<int>(a2a::error_code::invalid_params),
  ...
  require(invalid_id.at("id").is_null() &&
            invalid_id.at("error").at("code") == static_cast<int>(a2a::error_code::invalid_request),
  ...
  require(!unsupported_mode &&
            unsupported_mode.failure->code == a2a::error_code::content_type_not_supported,
...
void remote_and_local_team_adapters_bridge_a2a() {
  auto remote_service =
    std::make_shared<a2a::service>(test_card(), std::make_shared<memory_handler>());
  auto remote_client =
    std::make_shared<a2a::client>(std::make_shared<a2a::in_process_transport>(remote_service));
  auto registry = std::make_shared<ma::agent_registry>();
  registry->add(a2a::agent_descriptor_from_card(test_card(), "remote-reviewer"),
  ...
  require(exposed && exposed.value->artifacts.front().parts.front().text == "local:input" &&
            ...
            observed_metadata.at("attempt") == "7" && observed_metadata.at("tenant") == "acme",
...
void adapter_capabilities_and_function_handlers_are_conservative() {
  auto conservative_client =
    std::make_shared<a2a::client>(std::make_shared<conservative_transport>());
  ...
  auto concurrent_client =
    std::make_shared<a2a::client>(std::make_shared<a2a::in_process_transport>(
      std::make_shared<a2a::service>(test_card(), std::make_shared<memory_handler>())));
...
void input_required_tasks_can_continue_or_cancel() {
  ...
  require(paused_for_cancel &&
            paused_for_cancel.value->status.state == a2a::task_state::input_required && cancelled &&
            cancelled.value->status.state == a2a::task_state::canceled,
...
void in_process_transport_honors_pre_cancelled_requests() {
  ...
  auto handler = std::make_shared<a2a::function_task_handler>(
    ...
    [](const a2a::task_query_params&) { return a2a::result<a2a::task> { .value = a2a::task {} }; },
    [](const a2a::task_id_params&) { return a2a::result<a2a::task> { .value = a2a::task {} }; });
  a2a::client client(std::make_shared<a2a::in_process_transport>(
...
void handler_destruction_cancels_background_work() {
  ...
  registry->add({ .id = "cooperative", .name = "cooperative" },
    std::make_shared<ma::function_agent_executor>(
      [state](const ma::agent_task_request&, const ma::agent_execution_context& context) {
        state->entered = true;
        while (!context.stop_token.stop_requested()) {
          std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }
  ...
  auto handler = std::make_unique<a2a::team_task_handler>(a2a::team_task_handler_options {
    .runtime = runtime,
  ...
  require(
    static_cast<bool>(submitted), "background task should be accepted before handler shutdown");
  while (!state->entered)
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  handler.reset();
  const auto cancellation_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(2);
  while (!state->stopped && std::chrono::steady_clock::now() < cancellation_deadline) {
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
  require(
    state->stopped, "handler destruction must request cancellation for accepted background work");
}
...
int main() {
  try {
    ...
    run("input-required tasks can continue or cancel", input_required_tasks_can_continue_or_cancel);
    ...
    run("handler destruction cancels background work", handler_destruction_cancels_background_work);
  }
