...
public:
  wuwe::llm_response complete(const wuwe::llm_request& request) override {
    return { .content = request.messages.empty() ? std::string {} : request.messages.back().content };
  }
...
void fan_out_and_fan_in_compose_inside_a_flow() {
  ...
  require(pipeline.invoke(10) == 36,
    "fan_out and fan_in are first-class typed flow steps");
}
...
void fan_out_each_supports_runtime_sized_workloads() {
  ...
  require(result && result.items.size() == 4 &&
      *result.items[0].value == 4 && *result.items[1].value == 10 &&
      *result.items[2].value == 18 && *result.items[3].value == 28,
...
void fan_out_each_reuses_a_bounded_worker_set() {
  ...
  auto parallel_map = wuwe::fan_out_each(
    wuwe::fan_out_options { .max_concurrency = 3 },
    [&](const int& value) {
      {
        std::scoped_lock lock(thread_ids_mutex);
        if (std::find(thread_ids.begin(), thread_ids.end(),
              std::this_thread::get_id()) == thread_ids.end()) {
  ...
  require(result && result.completed_count == 64 &&
      !thread_ids.empty() && thread_ids.size() <= 3,
...
void fail_fast_returns_without_waiting_for_uncooperative_branches() {
  ...
  auto scatter = wuwe::fan_out(
    ...
    [&](const int&) -> int {
      while (!sibling_entered) std::this_thread::sleep_for(1ms);
      throw std::runtime_error("stop the group");
    },
    [&](const int&, const wuwe::fan_out_context& context) {
      std::stop_callback callback(context.stop_token, [&] {
        cancellation_observed = true;
      });
      sibling_entered = true;
      while (!release_sibling) std::this_thread::sleep_for(1ms);
  ...
  while (!sibling_finished) std::this_thread::sleep_for(1ms);
}

void timeout_and_external_cancellation_are_prompt() {
  std::atomic<bool> timeout_release { false };
  std::atomic<bool> timeout_finished { false };
  auto timed = wuwe::fan_out(
    ...
    [&](const int&, const wuwe::fan_out_context& context) {
      ...
      while (!timeout_release) std::this_thread::sleep_for(1ms);
  ...
  std::jthread canceller([&] {
    while (!cancel_entered) std::this_thread::sleep_for(1ms);
    stop_source.request_stop();
  });
  ...
  cancel_release = true;
  while (!cancel_finished) std::this_thread::sleep_for(1ms);
}

void void_and_move_only_results_are_supported() {
  std::atomic<int> side_effects { 0 };
  auto void_result = wuwe::fan_out(
    [&](const int&) { ++side_effects; },
    [&](const int&) { ++side_effects; }).run(0);
  ...
  auto move_only = wuwe::fan_out(
    [](const int& value) { return std::make_unique<int>(value + 1); },
    [](const int& value) { return std::make_unique<int>(value + 2); }).run(5);
  auto joined = wuwe::fan_in_all()(std::move(move_only));
  require(*joined[0] == 6 && *joined[1] == 7,
    "fan_out and fan_in preserve move-only result types");
}
...
void invalid_options_are_rejected() {
  ...
  try {
    (void)wuwe::fan_out(
      wuwe::fan_out_options { .max_concurrency = 0 },
  ...
  try {
    (void)wuwe::fan_out(
      wuwe::fan_out_options { .timeout = -1ms },
      [](const int& value) { return value; });
  }
  catch (const std::invalid_argument&) {
...
int main() {
  try {
    ...
    run("fan_out and fan_in compose inside a flow",
      fan_out_and_fan_in_compose_inside_a_flow);
    run("fan_out_each supports runtime-sized workloads",
      fan_out_each_supports_runtime_sized_workloads);
    run("fan_out_each reuses a bounded worker set",
      fan_out_each_reuses_a_bounded_worker_set);
