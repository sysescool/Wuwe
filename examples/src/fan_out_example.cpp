...
int main() {
  ...
  auto analyze = wuwe::fan_out(
    ...
    [](const std::string& text) { return "length=" + std::to_string(text.size()); },
    [](const std::string& text) { return "first=" + std::string(1, text.front()); },
    [](const std::string& text) { return "last=" + std::string(1, text.back()); });
  ...
  auto parallel_map = wuwe::fan_out_each(
    wuwe::fan_out_options { .max_concurrency = 2 }, [](const int& value) { return value * value; });
  const auto squares = wuwe::fan_in_all()(parallel_map.run(std::vector<int> { 1, 2, 3, 4 }));
