...
[[nodiscard]] inline std::optional<llm_cost_breakdown> calculate_llm_cost(
  const llm_usage& usage,
  const llm_pricing& pricing) noexcept {
  if (!valid_llm_pricing(pricing) || usage.prompt_tokens < 0 ||
  ...
  const auto cached_rate = pricing.cached_input_per_million_tokens_usd.value_or(
    pricing.input_per_million_tokens_usd);
  const auto reasoning_rate = pricing.reasoning_per_million_tokens_usd.value_or(
  ...
  result.total_usd = saturated_add(
    saturated_add(result.input_usd, result.cached_input_usd),
...
inline void accumulate_llm_usage(llm_usage& target, const llm_usage& value) noexcept {
  ...
  target.total_tokens = (std::max)(
    target.total_tokens,
    add(target.prompt_tokens, target.completion_tokens));
  target.cached_prompt_tokens = add(
