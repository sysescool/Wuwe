...
inline void saturating_context_token_add(
  std::size_t& target,
...
public:
  ...
  [[nodiscard]] virtual std::string truncate_text(
    ...
    bool keep_tail) const = 0;

  [[nodiscard]] virtual std::size_t estimate_message(
    const ::wuwe::chat_message& message) const {
    std::size_t tokens = 4;
    ...
    for (const auto& call : message.tool_calls) {
      ...
      detail::saturating_context_token_add(
        tokens, estimate_text(call.arguments_json));
    }
    return tokens;
  }

  [[nodiscard]] virtual std::size_t estimate_tool(
    const ::wuwe::llm_tool& tool) const {
    std::size_t tokens = 8;
    ...
    detail::saturating_context_token_add(
      tokens, estimate_text(tool.parameters_json_schema));
    return tokens;
  }
};
...
public:
  [[nodiscard]] std::size_t estimate_text(std::string_view text) const override {
    ...
    for (std::size_t index = 0; index < text.size();) {
      ...
      if (byte < 0x80) {
        ...
        if ((byte & 0xe0) == 0xc0) index += 2;
        else if ((byte & 0xf0) == 0xe0) index += 3;
        else if ((byte & 0xf8) == 0xf0) index += 4;
        else ++index;
  ...
  [[nodiscard]] std::string truncate_text(
    ...
    if (estimate_text(text) <= token_limit) return std::string(text);
    if (token_limit == 0) return {};
    std::size_t low = 0;
    std::size_t high = text.size();
    while (low < high) {
      const auto length = low + (high - low + 1) / 2;
      const auto candidate = keep_tail
        ? valid_tail(text, length)
        : valid_prefix(text, length);
      if (estimate_text(candidate) <= token_limit) low = length;
      else high = length - 1;
    }
...
private:
  [[nodiscard]] static std::string_view valid_prefix(
    std::string_view text,
    std::size_t length) {
  ...
  [[nodiscard]] static std::string_view valid_tail(
    std::string_view text,
    std::size_t length) {
    length = (std::min)(length, text.size());
...
struct context_budget_result {
  ...
  explicit operator bool() const noexcept { return report.fitted; }
};
...
[[nodiscard]] inline llm_context_source resolved_context_source(
  ...
  if (message.role == "system") return llm_context_source::system;
...
public:
  explicit context_budget_manager(
    std::shared_ptr<const context_token_estimator> estimator = {})
  ...
  [[nodiscard]] context_budget_result fit(
    ::wuwe::llm_request request,
    const ::wuwe::llm_context_budget& budget) const {
    ...
    if (result.request.max_output_tokens &&
        *result.request.max_output_tokens <= 0) {
      result.report.error = "max output tokens must be positive when specified";
      return result;
    }
    const auto requested_output = result.request.max_output_tokens &&
        *result.request.max_output_tokens > 0
      ? static_cast<std::size_t>(*result.request.max_output_tokens)
      : std::size_t { 0 };
    const auto reserved_output = (std::max)(
      budget.reserved_output_tokens, requested_output);
    ...
    if (!apply_declared_limits(result, removed, budget)) return result;
    result.report.after = usage(result.request, removed, reserved_output);
    const auto immutable_tokens = detail::saturating_context_token_sum(
      result.report.after.tool_schemas,
      budget.allow_system_truncation ? 0 : result.report.after.system);
    if (immutable_tokens > input_limit) {
      result.report.error =
        "protected system context and tool schemas exceed the input budget";
      return result;
    }
    ...
    for (const auto source : reduction_order) {
      if (result.report.after.input_total() <= input_limit) break;
      ...
      const auto preserve = source == llm_context_source::conversation
        ? budget.minimum_recent_conversation_messages : 0;
      (void)reduce(result, removed, source, target, true, preserve);
      result.report.after = usage(result.request, removed, reserved_output);
    }

    if (result.report.after.input_total() > input_limit &&
        budget.allow_system_truncation) {
      const auto excess = result.report.after.input_total() - input_limit;
      const auto target = result.report.after.system > excess
        ? result.report.after.system - excess : 0;
      (void)reduce(
        result, removed, llm_context_source::system, target, true, 0);
      result.report.after = usage(result.request, removed, reserved_output);
    }
    if (result.report.after.input_total() > input_limit) {
    ...
    for (std::size_t index = 0; index < result.request.messages.size(); ++index) {
      if (!removed[index]) fitted.push_back(std::move(result.request.messages[index]));
    }
...
private:
  [[nodiscard]] bool apply_declared_limits(
    context_budget_result& result,
    ...
    const struct limit_entry {
      ...
      { llm_context_source::system, budget.limits.system,
        budget.allow_system_truncation, 0 },
      ...
      { llm_context_source::conversation, budget.limits.conversation, true,
    ...
    for (const auto& entry : entries) {
      ...
      if (!reduce(result, removed, entry.source, entry.limit,
            entry.truncate, entry.preserve)) {
  ...
  [[nodiscard]] context_budget_usage usage(
    ...
    for (const auto& tool : request.tools) {
      detail::saturating_context_token_add(
        output.tool_schemas, estimator_->estimate_tool(tool));
    }
    return output;
  }
  ...
  [[nodiscard]] bool reduce(
    ...
    auto current = component(usage(result.request, removed,
      result.report.before.reserved_output), source);
    ...
    for (std::size_t index = 0; index < result.request.messages.size(); ++index) {
      if (!removed[index] &&
          resolved_context_source(result.request.messages[index]) == source) {
        candidates.push_back(index);
      }
    }
    const auto removable = candidates.size() > preserve_recent
      ? candidates.size() - preserve_recent : 0;
    for (std::size_t position = 0; position < candidates.size() && current > limit;
         ++position) {
      ...
      else if (current - (std::min)(current, tokens) >= limit ||
               !can_truncate(message)) {
        removed[index] = true;
      ...
      current = component(usage(result.request, removed,
        result.report.before.reserved_output), source);
    }
  ...
  static void remove_tool_exchange(
    ...
    for (std::size_t index = assistant_index + 1;
         index < result.request.messages.size(); ++index) {
      const auto& message = result.request.messages[index];
      if (resolved_context_source(message) != llm_context_source::tool_result) {
        break;
      }
      if (!message.tool_call_id ||
          std::find(call_ids.begin(), call_ids.end(), *message.tool_call_id) ==
            call_ids.end()) {
        break;
      }
      remove(index);
  ...
  void truncate_message(
    ::wuwe::chat_message& message,
    std::size_t tokens_to_remove) const {
    const auto content_tokens = estimator_->estimate_text(message.content);
    const auto target = content_tokens > tokens_to_remove
      ? content_tokens - tokens_to_remove : 0;
    const bool keep_tail = resolved_context_source(message) ==
      llm_context_source::conversation;
    message.content = estimator_->truncate_text(message.content, target, keep_tail);
  }

  [[nodiscard]] static std::size_t component(
    const context_budget_usage& usage,
    llm_context_source source) {
    switch (source) {
      case llm_context_source::system: return usage.system;
      case llm_context_source::conversation: return usage.conversation;
      case llm_context_source::memory: return usage.memory;
      case llm_context_source::knowledge: return usage.knowledge;
      case llm_context_source::tool_result: return usage.tool_results;
      case llm_context_source::other:
      case llm_context_source::automatic: return usage.other;
    }
    return usage.other;
  }
