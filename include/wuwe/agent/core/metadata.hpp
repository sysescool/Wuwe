...
[[nodiscard]] inline bool sensitive_metadata_key(
  ...
  for (const auto raw_ch : key) {
    ...
    if (std::isalnum(ch)) {
      if (std::isupper(ch) && previous_was_lower_or_digit &&
  ...
  constexpr std::string_view sensitive_names[] {
    ...
    "access_key", "private_key", "secret_key", "cookie", "set_cookie",
  };
  ...
  constexpr std::string_view sensitive_suffixes[] {
    "_token", ".token", "_secret", ".secret", "_password", ".password",
    "_credential", ".credential", "_credentials", ".credentials",
    "_authorization", ".authorization", "_api_key", ".api_key",
    "_access_key", ".access_key", "_private_key", ".private_key",
    "_secret_key", ".secret_key", "_cookie", ".cookie",
  };
...
[[nodiscard]] inline bool sensitive_execution_context_metadata_key(
  std::string_view key) noexcept {
