...
inline void validate_sqlite_table(sqlite3* db, const std::string& table,
  std::initializer_list<sqlite_column_contract> required_columns, const std::string& component) {
  if (!db) {
  ...
  if (table.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_") !=
      std::string::npos) {
  ...
  while (true) {
    ...
    columns.emplace(detail::sqlite_column_text(raw_statement, 1),
      detail::sqlite_column_info {
        .declared_type =
          detail::normalized_sqlite_type(detail::sqlite_column_text(raw_statement, 2)),
  ...
  for (const auto& required : required_columns) {
    ...
    if (found == columns.end()) {
      throw std::runtime_error(component + " table " + table + " is missing required column " +
                               (required.name ? required.name : "<unnamed>"));
    }
    const auto expected_type =
      detail::sqlite_type_affinity(required.declared_type ? required.declared_type : "");
    const auto actual_type = detail::sqlite_type_affinity(found->second.declared_type);
    if (required.declared_type && *required.declared_type != '\0' && actual_type != expected_type) {
      throw std::runtime_error(component + " table " + table + " column " + required.name +
                               " has " + actual_type + " affinity, expected " + expected_type);
    }
    if (required.not_null && !found->second.not_null && found->second.primary_key_position == 0) {
      throw std::runtime_error(
        component + " table " + table + " column " + required.name + " must be NOT NULL");
    }
  ...
  for (const auto& [name, column] : columns) {
    if (column.primary_key_position == 0)
      continue;
    const auto expected = std::find_if(required_columns.begin(),
