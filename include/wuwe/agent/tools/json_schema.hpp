...
private:
  ...
  static std::optional<std::size_t> nonnegative_size(
    const nlohmann::json& value) {
    if (value.is_number_unsigned()) return value.get<std::size_t>();
  ...
  static bool validate_type_keyword(
    const nlohmann::json& instance,
    const nlohmann::json& keyword) {
    ...
    if (keyword.is_array()) {
      for (const auto& item : keyword) {
        if (item.is_string() &&
            matches_type(instance, item.get_ref<const std::string&>())) {
          return true;
        }
      }
    }
  ...
  static const nlohmann::json* resolve_local_ref(
    ...
    if (!reference.starts_with("#/")) return nullptr;
    try {
  ...
  void validate_node(
    ...
    std::size_t depth,
    json_schema_validation_result& result) const {
    ...
    for (const auto* keyword : unsupported_assertions) {
      if (schema.contains(keyword)) {
        add_issue(result, instance_path, schema_path + "/" + keyword,
    ...
    if (const auto ref = schema.find("$ref"); ref != schema.end()) {
      ...
      if (!target) {
        add_issue(result, instance_path, schema_path + "/$ref",
          "only valid local JSON Pointer references are supported");
        return;
      }
      validate_node(instance, *target, root, instance_path,
        ref->get_ref<const std::string&>(), depth + 1, result);
    }

    if (const auto type = schema.find("type"); type != schema.end() &&
        !validate_type_keyword(instance, *type)) {
      add_issue(result, instance_path, schema_path + "/type",
        "value does not match the declared type");
      return;
    }
    if (instance.is_number_float() &&
        !std::isfinite(instance.get<double>())) {
      add_issue(result, instance_path, schema_path,
        "JSON numbers must be finite");
      return;
    }
    if (const auto constant = schema.find("const"); constant != schema.end() &&
        instance != *constant) {
      add_issue(result, instance_path, schema_path + "/const",
    ...
    if (const auto values = schema.find("enum"); values != schema.end()) {
      if (!values->is_array()) {
        ...
        if (!found) add_issue(result, instance_path, schema_path + "/enum",
    ...
    if (instance.is_string()) {
      validate_string(instance.get_ref<const std::string&>(), schema,
  ...
  void validate_combinators(
    ...
    for (const auto* keyword : { "allOf", "anyOf", "oneOf" }) {
      ...
      if (alternatives == schema.end()) continue;
      ...
      for (std::size_t index = 0; index < alternatives->size(); ++index) {
        ...
        validate_node(instance, alternatives->at(index), root, instance_path,
          schema_path + "/" + keyword + "/" + std::to_string(index),
          depth + 1, nested);
        if (nested.valid()) ++matches;
      }
      const bool valid = std::string_view(keyword) == "allOf"
        ? matches == alternatives->size()
        : std::string_view(keyword) == "anyOf" ? matches > 0 : matches == 1;
      if (!valid) add_issue(result, instance_path, schema_path + "/" + keyword,
        std::string("value does not satisfy ") + keyword);
    }
    if (const auto negated = schema.find("not"); negated != schema.end()) {
      json_schema_validation_result nested;
      validate_node(instance, *negated, root, instance_path,
        schema_path + "/not", depth + 1, nested);
      if (nested.valid()) add_issue(result, instance_path, schema_path + "/not",
        "value satisfies a negated schema");
    }
  }
  ...
  void validate_object(
    ...
    std::size_t depth,
    json_schema_validation_result& result) const {
    validate_size_keyword(instance.size(), schema, "minProperties", true,
      instance_path, schema_path, "object has too few properties", result);
    validate_size_keyword(instance.size(), schema, "maxProperties", false,
      instance_path, schema_path, "object has too many properties", result);
    if (const auto required = schema.find("required"); required != schema.end()) {
      ...
      else for (const auto& property : *required) {
        if (!property.is_string()) {
          add_issue(result, instance_path, schema_path + "/required",
    ...
    if (properties != schema.end()) {
      if (!properties->is_object()) {
        add_issue(result, instance_path, schema_path + "/properties",
          "properties must be an object");
      }
      else for (const auto& [name, child_schema] : properties->items()) {
        if (!instance.contains(name)) continue;
        evaluated.insert(name);
        validate_node(instance.at(name), child_schema, root,
          instance_path + "/" + escape_pointer(name),
          schema_path + "/properties/" + escape_pointer(name), depth + 1, result);
      }
    }
    ...
    if (additional != schema.end()) {
      if (!additional->is_object() && !additional->is_boolean()) {
        add_issue(result, instance_path, schema_path + "/additionalProperties",
      ...
      for (const auto& [name, value] : instance.items()) {
        ...
        if (additional->is_boolean() && !additional->get<bool>()) {
          add_issue(result, instance_path + "/" + escape_pointer(name),
            schema_path + "/additionalProperties", "additional property is not allowed");
        }
        else if (additional->is_object() || additional->is_boolean()) {
          validate_node(value, *additional, root,
            instance_path + "/" + escape_pointer(name),
            schema_path + "/additionalProperties", depth + 1, result);
        }
      }
    }
  }
  ...
  void validate_array(
    ...
    std::size_t depth,
    json_schema_validation_result& result) const {
    validate_size_keyword(instance.size(), schema, "minItems", true,
      instance_path, schema_path, "array has too few items", result);
    validate_size_keyword(instance.size(), schema, "maxItems", false,
      instance_path, schema_path, "array has too many items", result);
    const auto unique = schema.find("uniqueItems");
    if (unique != schema.end() && !unique->is_boolean()) {
      add_issue(result, instance_path, schema_path + "/uniqueItems",
    ...
    if (const auto items = schema.find("items"); items != schema.end()) {
      for (std::size_t index = 0; index < instance.size(); ++index) {
        validate_node(instance[index], *items, root,
          instance_path + "/" + std::to_string(index), schema_path + "/items",
          depth + 1, result);
      }
    }
  }
  ...
  static void validate_string(
    ...
    validate_size_keyword(length, schema, "minLength", true,
      instance_path, schema_path, "string is too short", result);
    validate_size_keyword(length, schema, "maxLength", false,
      instance_path, schema_path, "string is too long", result);
    if (const auto pattern = schema.find("pattern"); pattern != schema.end()) {
      if (!pattern->is_string()) {
        ...
        if (!std::regex_search(instance, std::regex(pattern->get<std::string>()))) {
          add_issue(result, instance_path, schema_path + "/pattern",
  ...
  static std::size_t utf8_length(std::string_view value) {
    ...
    for (const auto byte : value) {
      if ((static_cast<unsigned char>(byte) & 0xc0) != 0x80) ++length;
    }
    return length;
  }

  static void validate_size_keyword(
    std::size_t actual,
    const nlohmann::json& schema,
    const char* keyword,
    bool minimum,
  ...
  static void validate_number(
    ...
    compare("exclusiveMinimum", std::greater<double> {},
      "number is not above exclusiveMinimum");
    compare("exclusiveMaximum", std::less<double> {},
    ...
    if (const auto multiple = schema.find("multipleOf"); multiple != schema.end()) {
      if (!multiple->is_number() ||
          !std::isfinite(multiple->get<double>()) ||
          multiple->get<double>() <= 0.0) {
        add_issue(result, instance_path, schema_path + "/multipleOf",
