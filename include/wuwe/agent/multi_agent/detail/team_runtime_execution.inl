...
inline std::optional<detail::team_runtime_slot> team_runtime::acquire_runtime_slot(
  ...
  const auto acquired = deadline
    ? state_->concurrency_condition.wait_until(lock, stop_token, *deadline, available)
...
inline detail::agent_execution_outcome team_runtime::execute_agent(
  agent_lease lease,
  ...
  for (;;) {
    const auto wait_for = deadline
      ? (std::min)(state_->options.cancellation_poll_interval,
