...
using plan_priority_scorer =
  std::function<double(const plan_step&, const plan&, std::chrono::system_clock::time_point)>;
...
public:
  explicit plan_prioritizer(
    plan_prioritization_policy policy = {}, plan_priority_scorer scorer = {})
      : policy_(std::move(policy)), scorer_(std::move(scorer)) {
    validate_policy(policy_);
  }

  [[nodiscard]] prioritized_plan_step evaluate(const plan& value, std::size_t index,
    ...
    const auto score = scorer_ ? scorer_(step, value, now) : default_score(step, now);
  ...
  [[nodiscard]] std::vector<std::size_t> order(const plan& value, std::vector<std::size_t> indices,
    std::chrono::system_clock::time_point now = std::chrono::system_clock::now()) const {
    if (!policy_.enabled || indices.size() < 2)
      return indices;

    std::vector<prioritized_plan_step> ranked;
    ranked.reserve(indices.size());
    for (const auto index : indices)
      ranked.push_back(evaluate(value, index, now));
...
private:
  [[nodiscard]] double default_score(
    const plan_step& step, std::chrono::system_clock::time_point now) const noexcept {
    double score = step.priority * policy_.priority_weight + step.urgency * policy_.urgency_weight +
                   step.expected_value * policy_.expected_value_weight -
                   step.estimated_cost * policy_.estimated_cost_weight;
    if (!step.deadline || policy_.deadline_weight == 0.0)
      return score;

    if (*step.deadline <= now) {
      return score + policy_.deadline_weight;
    }
    if (policy_.deadline_horizon.count() == 0)
      return score;
    const auto remaining = std::chrono::duration<double>(*step.deadline - now).count();
    const auto horizon = std::chrono::duration<double>(policy_.deadline_horizon).count();
  ...
  static void validate_policy(const plan_prioritization_policy& policy) {
    ...
    if (!finite(policy.priority_weight) || !finite(policy.urgency_weight) ||
        !finite(policy.expected_value_weight) || !finite(policy.estimated_cost_weight) ||
    ...
    if (policy.priority_weight < 0.0 || policy.urgency_weight < 0.0 ||
        policy.expected_value_weight < 0.0 || policy.estimated_cost_weight < 0.0 ||
        policy.deadline_weight < 0.0) {
      throw std::invalid_argument("plan prioritization weights must be non-negative");
    }
    if (policy.deadline_horizon.count() < 0) {
      throw std::invalid_argument("plan prioritization deadline horizon must not be negative");
    }
  }
