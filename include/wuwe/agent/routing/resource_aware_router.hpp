...
struct resource_aware_router_options {
  ...
  routing_telemetry_failure_mode telemetry_failure_mode {
    routing_telemetry_failure_mode::ignore
  };
};
...
public:
  explicit resource_aware_router(resource_aware_router_options options = {})
    ...
    if (!std::isfinite(options_.quality_weight) ||
        !std::isfinite(options_.latency_weight) ||
        !std::isfinite(options_.cost_weight) ||
        !std::isfinite(options_.preferred_model_bonus) ||
        options_.quality_weight < 0.0 || options_.latency_weight < 0.0 ||
        options_.cost_weight < 0.0 || options_.preferred_model_bonus < 0.0) {
      throw std::invalid_argument(
        "resource-aware routing weights must be finite and non-negative");
    }
  }

  resource_aware_router& add(model_resource_profile profile) {
    validate_profile(profile);
    if (find(profile.model)) {
      throw std::invalid_argument("duplicate model resource profile: " + profile.model);
    }
  ...
  [[nodiscard]] std::optional<model_resource_profile> find(
    ...
    return found == profiles_.end() ? std::nullopt
                                    : std::optional<model_resource_profile>(*found);
  }

  [[nodiscard]] model_route_decision route(
    const model_route_request& request) const override {
    if (!std::isfinite(request.max_estimated_cost_usd) ||
    ...
    if (eligible.empty()) {
      const auto cost_only_rejection = std::any_of(
        decision.candidates.begin(), decision.candidates.end(), [](const auto& candidate) {
          return !candidate.rejection_reasons.empty() &&
                 std::all_of(
                   candidate.rejection_reasons.begin(),
      ...
      const auto cost_budget_failure =
        request.max_estimated_cost_usd > 0.0 && cost_only_rejection;
      decision.error = cost_budget_failure
                         ? model_route_error_code::estimated_cost_budget_exceeded
...
private:
  static void validate_profile(const model_resource_profile& profile) {
    ...
    if (!std::isfinite(profile.quality_score) ||
        !std::isfinite(profile.latency_score) ||
        profile.quality_score < 0.0 || profile.quality_score > 1.0 ||
        profile.latency_score < 0.0 || profile.latency_score > 1.0) {
      throw std::invalid_argument(
  ...
  void evaluate_eligibility(
    ...
    const model_route_request& request,
    model_route_candidate& candidate) const {
    ...
    const bool pricing_required = request.max_estimated_cost_usd > 0.0 ||
    ...
    if (pricing_required && !candidate.estimated_cost_usd &&
        !required.allow_unpriced_models) {
  ...
  void score_candidates(
    ...
    for (const auto index : eligible) {
      ...
      const auto cost = candidate.estimated_cost_usd.value_or(
        maximum_cost > 0.0 ? maximum_cost : 0.0);
  ...
  void publish(model_route_decision& decision) const {
    ...
    if (options_.event_sink) {
      const auto trace_id = decision.metadata.contains("trace_id")
                              ? decision.metadata.at("trace_id")
                              : std::string {};
