...
public:
  ...
  virtual result<task> send(const send_message_params& params, std::stop_token stop_token) = 0;
...
public:
  using send_callback = std::function<result<task>(const send_message_params&, std::stop_token)>;
  ...
  function_task_handler(send_callback send, get_callback get, cancel_callback cancel)
      : send_(std::move(send)), get_(std::move(get)), cancel_(std::move(cancel)) {
    if (!send_ || !get_ || !cancel_) {
      throw std::invalid_argument("function_task_handler requires send, get, and cancel callbacks");
    }
  }

  result<task> send(const send_message_params& params, std::stop_token stop_token) override {
...
public:
  ...
  [[nodiscard]] nlohmann::json handle_jsonrpc(
    const nlohmann::json& request, std::stop_token stop_token = {}) const {
    ...
    try {
      ...
      if (!notification && !id.is_null() && !id.is_string() && !id.is_number_integer() &&
      ...
      if (method == "message/send") {
        ...
        if (!send_params.configuration.accepted_output_modes.empty() &&
            std::none_of(send_params.configuration.accepted_output_modes.begin(),
              send_params.configuration.accepted_output_modes.end(),
              [&](const auto& accepted) {
                return std::find(card_.default_output_modes.begin(),
      ...
      if (!handled) {
        return respond(error_response(id,
          handled.failure.value_or(error {
...
private:
  ...
  static task_query_params task_query_from_json(const nlohmann::json& value) {
    ...
    if (output.id.empty())
      throw std::invalid_argument("tasks/get requires id");
    return output;
  }
  ...
  static task_id_params task_id_from_json(const nlohmann::json& value) {
    ...
    if (output.id.empty())
      throw std::invalid_argument("tasks/cancel requires id");
    return output;
  }
...
public:
  explicit in_process_transport(std::shared_ptr<service> service) : service_(std::move(service)) {
    if (!service_) {
  ...
  rpc_result invoke(
    ...
    if (response.contains("error")) {
      ...
      return { .failure = error {
                 ...
                 .data = body.contains("data") ? std::optional<nlohmann::json>(body.at("data"))
                                               : std::nullopt,
               } };
    }
