...
public:
  ...
  [[nodiscard]] host_response_envelope dispatch(
    const host_request_envelope& request) const {
    if (!supports_protocol_version(request.call.protocol_version)) {
      return failure(request,
        host_error_code::unsupported_protocol_version,
        "unsupported agent host protocol version", false, {
    ...
    try {
      ...
      return failure(request, host_error_code::invalid_request,
        "malformed agent host request");
    }
    catch (...) {
      return failure(request, host_error_code::internal,
        "agent host request validation failed");
    }
    ...
    switch (request.operation) {
      ...
      case host_operation::get_run:
        return dispatch_operation(request,
          ...
          [&](const get_run_request& decoded) {
            return service_.get_run(request.call, decoded);
          }, run_view_to_json);
  ...
  [[nodiscard]] nlohmann::json dispatch_json(
    ...
    try {
      ...
      return host_response_to_json(failure(
        request_id, operation, host_error_code::invalid_request, ex.what()));
    }
    catch (const nlohmann::json::exception&) {
      return host_response_to_json(failure(
        request_id, operation, host_error_code::invalid_request,
        "malformed agent host request"));
    }
    catch (...) {
      return host_response_to_json(failure(
        request_id, operation, host_error_code::internal,
        "agent host request could not be dispatched"));
    }
  }

private:
  host_response_envelope dispatch_event_page(
    const host_request_envelope& request) const {
    ...
    try {
      ...
      return failure(request, host_error_code::invalid_request,
        "malformed list_events request");
    }
    catch (...) {
      return failure(request, host_error_code::internal,
        "agent host list_events request decoding failed");
    }
    try {
      ...
      return failure(request, host_error_code::internal,
        "agent host service failed to list events");
    }
  }

  template<typename Decoder, typename Invoker, typename Encoder>
  host_response_envelope dispatch_operation(
    ...
    try {
      ...
      try {
        ...
        return dispatch_result(
          request, std::move(result), std::forward<Encoder>(encode));
      }
      catch (...) {
        return failure(request, host_error_code::internal,
      ...
      return failure(request, host_error_code::invalid_request,
        "malformed agent host operation body");
    }
    catch (...) {
      return failure(request, host_error_code::internal,
        "agent host operation body decoding failed");
    }
  }
  ...
  static host_response_envelope failure(
    const host_request_envelope& request,
    host_error error) {
    return {
      .protocol_version = std::string(default_protocol_version),
  ...
  static host_response_envelope failure(
    ...
    return failure(request, { .code = code, .message = std::move(message),
      .retryable = retryable, .details = std::move(details) });
  }
