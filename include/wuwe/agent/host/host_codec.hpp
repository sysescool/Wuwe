...
inline std::string required_string(const nlohmann::json& body, const char* name) {
  const auto found = body.find(name);
  if (found == body.end() || !found->is_string() || found->get_ref<const std::string&>().empty()) {
...
inline std::uint64_t required_revision(const nlohmann::json& body) {
  ...
  if (found == body.end() || !found->is_number_unsigned()) {
    throw std::invalid_argument("agent host request requires unsigned 'expectedRevision'");
  }
...
inline runtime::approval_resolution approval_resolution_from_string(const std::string& value) {
  if (value == "approved")
    return runtime::approval_resolution::approved;
  if (value == "denied")
    return runtime::approval_resolution::denied;
  throw std::invalid_argument("agent host approval resolution must be 'approved' or 'denied'");
}

inline runtime::approval_resolution approval_state_from_string(const std::string& value) {
  if (value == "pending")
    return runtime::approval_resolution::pending;
...
[[nodiscard]] inline core::agent_execution_context execution_context_from_host_json(
  ...
  for (const auto& [key, _] : context.metadata) {
    if (core::sensitive_execution_context_metadata_key(key)) {
      throw std::invalid_argument(
        "agent host execution context must not contain sensitive metadata: " + key);
    }
  }
  if (value.contains("deadlineUnixMs") && !value.at("deadlineUnixMs").is_null()) {
    context.deadline =
      codec_detail::from_unix_millis(value.at("deadlineUnixMs").get<std::int64_t>());
  }
  return context;
}

[[nodiscard]] inline nlohmann::json create_run_request_to_json(const create_run_request& request) {
  if (request.context.run_id.empty()) {
    throw std::invalid_argument("host create_run requires a caller-assigned context.run_id");
  }
  return {
...
[[nodiscard]] inline create_run_request create_run_request_from_json(const nlohmann::json& body) {
  ...
  auto request = create_run_request {
    ...
    .metadata = body.value("metadata", std::map<std::string, std::string> {}),
  };
  if (request.context.run_id.empty()) {
    throw std::invalid_argument("host create_run requires a caller-assigned context.run_id");
  }
...
[[nodiscard]] inline nlohmann::json get_run_request_to_json(const get_run_request& request) {
...
[[nodiscard]] inline get_run_request get_run_request_from_json(const nlohmann::json& body) {
  return { .run_id = codec_detail::required_string(body, "runId") };
}

[[nodiscard]] inline nlohmann::json cancel_run_request_to_json(const cancel_run_request& request) {
  if (request.run_id.empty() || request.expected_revision == 0) {
    throw std::invalid_argument("cancel_run requires run_id and positive expected_revision");
  }
  return {
...
[[nodiscard]] inline cancel_run_request cancel_run_request_from_json(const nlohmann::json& body) {
  return {
...
[[nodiscard]] inline nlohmann::json resolve_approval_request_to_json(
  ...
  if (request.run_id.empty() || request.expected_revision == 0 || request.approval_id.empty() ||
...
[[nodiscard]] inline resolve_approval_request resolve_approval_request_from_json(
...
[[nodiscard]] inline nlohmann::json resume_run_request_to_json(const resume_run_request& request) {
  if (request.run_id.empty() || request.expected_revision == 0 || request.approval_id.empty()) {
    throw std::invalid_argument("resume_run requires run identity and approval identity");
  }
  return {
    { "runId", request.run_id },
    { "expectedRevision", request.expected_revision },
...
[[nodiscard]] inline resume_run_request resume_run_request_from_json(const nlohmann::json& body) {
  return {
    .run_id = codec_detail::required_string(body, "runId"),
    .expected_revision = codec_detail::required_revision(body),
    .approval_id = codec_detail::required_string(body, "approvalId"),
...
[[nodiscard]] inline nlohmann::json list_events_request_to_json(
  ...
  if (request.run_id.empty() || request.limit == 0 || request.limit > 1000) {
    throw std::invalid_argument("list_events requires run_id and a limit between 1 and 1000");
  }
  return {
    { "runId", request.run_id },
...
[[nodiscard]] inline list_events_request list_events_request_from_json(const nlohmann::json& body) {
...
[[nodiscard]] inline nlohmann::json run_submission_to_json(const run_submission& value) {
...
[[nodiscard]] inline run_submission run_submission_from_json(const nlohmann::json& value) {
  const auto run_id = codec_detail::required_string(value, "runId");
  const auto revision = value.value("revision", std::uint64_t {});
  if (revision == 0) {
    throw std::invalid_argument("agent host run submission requires a positive revision");
  }
  return {
    .run_id = run_id,
    .revision = revision,
    .status = runtime::agent_run_status_from_string(codec_detail::required_string(value, "status")),
  };
}

[[nodiscard]] inline nlohmann::json approval_view_to_json(const approval_view& value) {
  if (value.approval_id.empty() || value.tool_call_id.empty() || value.tool_name.empty()) {
...
[[nodiscard]] inline approval_view approval_view_from_json(const nlohmann::json& value) {
  if (!value.is_object()) {
    throw std::invalid_argument("invalid agent host approval view");
  }
  approval_view output {
    ...
    .resolution =
      codec_detail::approval_state_from_string(codec_detail::required_string(value, "resolution")),
    .reason = value.value("reason", std::string {}),
    .created_at = codec_detail::from_unix_millis(value.value("createdAtUnixMs", std::int64_t {})),
    .metadata = value.value("metadata", std::map<std::string, std::string> {}),
  };
  if (value.contains("resolvedAtUnixMs") && !value.at("resolvedAtUnixMs").is_null()) {
    output.resolved_at =
      codec_detail::from_unix_millis(value.at("resolvedAtUnixMs").get<std::int64_t>());
  }
  if (output.resolution == runtime::approval_resolution::pending && output.resolved_at) {
    throw std::invalid_argument("pending agent host approval cannot have a resolution timestamp");
  }
  if (output.resolution != runtime::approval_resolution::pending && !output.resolved_at) {
    throw std::invalid_argument("resolved agent host approval requires a resolution timestamp");
  }
  return output;
}
...
[[nodiscard]] inline nlohmann::json run_view_to_json(const run_view& value) {
  ...
  nlohmann::json output {
    ...
    { "approval",
      value.approval ? approval_view_to_json(*value.approval) : nlohmann::json(nullptr) },
    { "result", value.result },
    { "error", value.error },
    { "metadata", codec_detail::safe_metadata(value.metadata) },
  };
  output["completedAtUnixMs"] =
    value.completed_at ? nlohmann::json(codec_detail::to_unix_millis(*value.completed_at))
                       : nlohmann::json(nullptr);
  return output;
}
...
[[nodiscard]] inline run_view run_view_from_json(const nlohmann::json& value) {
  ...
  run_view output {
    ...
    .context = execution_context_from_host_json(*context),
    .created_at = codec_detail::from_unix_millis(value.value("createdAtUnixMs", std::int64_t {})),
    .updated_at = codec_detail::from_unix_millis(value.value("updatedAtUnixMs", std::int64_t {})),
    .result = value.value("result", nlohmann::json {}),
    .error = value.value("error", std::string {}),
    .metadata = value.value("metadata", std::map<std::string, std::string> {}),
  };
  if (output.revision == 0) {
    throw std::invalid_argument("agent host run view requires a positive revision");
  }
  if (output.context.run_id.empty()) {
    output.context.run_id = output.run_id;
  }
  else if (output.context.run_id != output.run_id) {
    throw std::invalid_argument("agent host run context does not match the run view identity");
  }
  if (value.contains("completedAtUnixMs") && !value.at("completedAtUnixMs").is_null()) {
    output.completed_at =
      codec_detail::from_unix_millis(value.at("completedAtUnixMs").get<std::int64_t>());
  }
...
[[nodiscard]] inline nlohmann::json host_event_to_json(const host_event& value) {
...
[[nodiscard]] inline host_event host_event_from_json(const nlohmann::json& value) {
  if (!value.is_object()) {
    throw std::invalid_argument("invalid agent host event");
  }
  host_event output {
    ...
    .timestamp = codec_detail::from_unix_millis(value.value("timestampUnixMs", std::int64_t {})),
    .data = value.value("data", nlohmann::json {}),
    .metadata = value.value("metadata", std::map<std::string, std::string> {}),
  };
  if (output.sequence == 0) {
    throw std::invalid_argument("agent host event requires a positive sequence");
  }
  return output;
}

[[nodiscard]] inline nlohmann::json event_page_to_json(const event_page& value) {
  auto events = nlohmann::json::array();
  std::uint64_t previous {};
  for (const auto& event : value.events) {
    if (event.sequence == 0 || (previous != 0 && event.sequence <= previous)) {
      throw std::invalid_argument("agent host event page must have strictly increasing sequences");
    }
    previous = event.sequence;
    events.push_back(host_event_to_json(event));
  }
  if (!value.events.empty() && value.next_sequence != previous) {
    throw std::invalid_argument("agent host event page cursor must equal the last event sequence");
  }
...
[[nodiscard]] inline event_page event_page_from_json(const nlohmann::json& value) {
  if (!value.is_object() || !value.contains("events") || !value.at("events").is_array()) {
