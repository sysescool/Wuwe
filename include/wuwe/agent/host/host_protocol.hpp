...
[[nodiscard]] inline std::string to_string(host_operation operation) {
  switch (operation) {
    case host_operation::create_run: return "create_run";
    case host_operation::get_run: return "get_run";
    case host_operation::cancel_run: return "cancel_run";
    case host_operation::resolve_approval: return "resolve_approval";
    case host_operation::resume_run: return "resume_run";
    case host_operation::list_events: return "list_events";
  }
  return "get_run";
}

[[nodiscard]] inline host_operation host_operation_from_string(
  std::string_view value) {
  if (value == "create_run") return host_operation::create_run;
  if (value == "get_run") return host_operation::get_run;
  if (value == "cancel_run") return host_operation::cancel_run;
  if (value == "resolve_approval") return host_operation::resolve_approval;
  if (value == "resume_run") return host_operation::resume_run;
  if (value == "list_events") return host_operation::list_events;
  throw std::invalid_argument("unsupported agent host operation");
}

[[nodiscard]] inline std::string to_string(host_error_code code) {
  switch (code) {
    case host_error_code::invalid_request: return "invalid_request";
    case host_error_code::unsupported_protocol_version:
      return "unsupported_protocol_version";
    case host_error_code::unauthorized: return "unauthorized";
    case host_error_code::forbidden: return "forbidden";
    case host_error_code::not_found: return "not_found";
    case host_error_code::conflict: return "conflict";
    case host_error_code::unavailable: return "unavailable";
    case host_error_code::internal: return "internal";
  }
  return "internal";
}

[[nodiscard]] inline host_error_code host_error_code_from_string(
  std::string_view value) {
  if (value == "invalid_request") return host_error_code::invalid_request;
  if (value == "unsupported_protocol_version") {
    return host_error_code::unsupported_protocol_version;
  }
  if (value == "unauthorized") return host_error_code::unauthorized;
  if (value == "forbidden") return host_error_code::forbidden;
  if (value == "not_found") return host_error_code::not_found;
  if (value == "conflict") return host_error_code::conflict;
  if (value == "unavailable") return host_error_code::unavailable;
  if (value == "internal") return host_error_code::internal;
  throw std::invalid_argument("unsupported agent host error code");
}

[[nodiscard]] inline bool supports_protocol_version(
  std::string_view version) noexcept {
  return std::find(supported_protocol_versions.begin(),
           supported_protocol_versions.end(), version) !=
...
[[nodiscard]] inline bool mutating(host_operation operation) noexcept {
  return operation != host_operation::get_run &&
         operation != host_operation::list_events;
}
...
inline void validate_host_request(const host_request_envelope& request) {
  ...
  if (mutating(request.operation) && request.call.idempotency_key.empty()) {
    throw std::invalid_argument(
      "mutating agent host request requires an idempotency_key");
  }
...
[[nodiscard]] inline nlohmann::json host_request_to_json(
  const host_request_envelope& request) {
...
[[nodiscard]] inline host_request_envelope host_request_from_json(
  ...
  if (version == value.end() || !version->is_string() ||
    ...
    throw std::invalid_argument(
      "agent host request requires a protocolVersion");
  }
...
[[nodiscard]] inline nlohmann::json host_error_to_json(
  const host_error& error) {
  std::map<std::string, std::string> metadata;
...
[[nodiscard]] inline host_error host_error_from_json(
  const nlohmann::json& value) {
  if (!value.is_object()) {
    throw std::invalid_argument("agent host error must be an object");
  }
  return {
    .code = host_error_code_from_string(
      value.value("code", std::string {})),
...
[[nodiscard]] inline nlohmann::json host_response_to_json(
  const host_response_envelope& response) {
  return {
    ...
    { "error", response.error
        ? host_error_to_json(*response.error)
        : nlohmann::json(nullptr) },
  };
}

[[nodiscard]] inline host_response_envelope host_response_from_json(
  const nlohmann::json& value) {
  if (!value.is_object()) {
  ...
  if (version == value.end() || !version->is_string() ||
      version->get_ref<const std::string&>().empty()) {
    throw std::invalid_argument(
      "agent host response requires a protocolVersion");
  }
  ...
  if (response.request_id.empty() && !response.error) {
    throw std::invalid_argument(
      "successful agent host response requires a requestId");
  }
  return response;
}
