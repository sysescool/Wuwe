...
nlohmann::json result_json(const filesystem_result& result) {
  ...
  if (!result.content.empty() || result.successful()) output["content"] = result.content;
...
std::error_code error_for(filesystem_status status) {
  switch (status) {
    case filesystem_status::ok: return {};
    case filesystem_status::not_found: return std::make_error_code(std::errc::no_such_file_or_directory);
    case filesystem_status::already_exists: return std::make_error_code(std::errc::file_exists);
    ...
    case filesystem_status::cancelled: return std::make_error_code(std::errc::operation_canceled);
    ...
    case filesystem_status::io_error: return std::make_error_code(std::errc::io_error);
  }
...
std::vector<llm_tool> filesystem_tool_provider::tools() const {
  ...
  if (policy.allow_read) {
    ...
    result.push_back(make_tool(tool_name("read_file"),
      ...
      object_schema({
        ...
        { "max_bytes", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_read_bytes } } },
      }, { "path" })));
    result.push_back(make_tool(tool_name("list_directory"),
      ...
      object_schema({
        ...
        { "max_depth", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_search_depth } } },
        { "max_entries", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_directory_entries } } },
      }, { "path" })));
    result.push_back(make_tool(tool_name("glob_files"),
      ...
      object_schema({
        ...
        { "pattern", { { "type", "string" }, { "minLength", 1 }, { "maxLength", policy.max_pattern_bytes } } },
        { "max_depth", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_search_depth } } },
        { "max_entries", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_directory_entries } } },
      }, { "path", "pattern" })));
    result.push_back(make_tool(tool_name("search_text"),
      ...
      object_schema({
        { "path", path_property("Root-relative directory to search.") },
        { "query", { { "type", "string" }, { "minLength", 1 }, { "maxLength", policy.max_pattern_bytes } } },
        { "file_pattern", { { "type", "string" }, { "default", "**" }, { "maxLength", policy.max_pattern_bytes } } },
        { "case_sensitive", { { "type", "boolean" }, { "default", true } } },
        { "max_files", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_directory_entries } } },
        { "max_results", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_search_results } } },
        { "max_output_bytes", { { "type", "integer" }, { "minimum", 1 }, { "maximum", policy.max_search_output_bytes } } },
      }, { "path", "query" })));
  }
  if (policy.allow_write) {
    result.push_back(make_tool(tool_name("write_file"),
      "Atomically create or replace one UTF-8 text file. expected_revision prevents stale overwrites.",
      object_schema({
        { "path", path_property("Root-relative file path.") },
  ...
  if (policy.allow_create_directory) result.push_back(make_tool(tool_name("create_directory"),
  ...
  if (policy.allow_copy) result.push_back(make_tool(tool_name("copy_path"),
    "Copy a file or an explicitly recursive directory inside the configured root.", transfer_schema()));
  if (policy.allow_move) result.push_back(make_tool(tool_name("move_path"),
    "Move a file or directory inside the configured root.", transfer_schema()));
  if (policy.allow_remove) result.push_back(make_tool(tool_name("remove_path"),
...
llm_tool_result filesystem_tool_provider::invoke(
  ...
  if (options_.max_arguments_bytes > 0 && arguments_json.size() > options_.max_arguments_bytes) {
    ...
    return { .content = "tool arguments are too large", .error_code = std::make_error_code(std::errc::value_too_large) };
  }
  try {
    ...
    if (name == tool_name("read_file")) return as_tool_result(runtime_.read_text({
      .path = args.at("path").get<std::string>(),
      .max_bytes = bounded_size(args, "max_bytes", 0),
    }, stop_token));
    if (name == tool_name("file_info")) return as_tool_result(runtime_.file_info({
      .path = args.at("path").get<std::string>(),
    ...
    if (name == tool_name("write_file")) {
      write_text_request request {
        ...
        .disposition = args.value("create_new", false) ? write_disposition::create_new : write_disposition::overwrite,
        .create_parent_directories = args.value("create_parent_directories", false),
      };
      if (args.contains("expected_revision")) request.expected_revision = args.at("expected_revision").get<std::string>();
    ...
    if (name == tool_name("list_directory")) return as_tool_result(runtime_.list_directory({
      .path = args.at("path").get<std::string>(),
    ...
    if (name == tool_name("glob_files")) return as_tool_result(runtime_.glob({
      .path = args.at("path").get<std::string>(),
      .pattern = args.at("pattern").get<std::string>(),
      .max_depth = bounded_size(args, "max_depth", 32),
      .max_entries = bounded_size(args, "max_entries", 1000),
    }, stop_token));
    if (name == tool_name("search_text")) return as_tool_result(runtime_.search_text({
      .path = args.at("path").get<std::string>(),
    ...
    if (name == tool_name("create_directory")) return as_tool_result(runtime_.create_directory({
      .path = args.at("path").get<std::string>(),
    ...
    return { .content = "tool not found: " + name, .error_code = std::make_error_code(std::errc::function_not_supported) };
  }
  catch (const std::exception& error) {
    runtime_.audit_tool_rejection(name, error.what());
    return { .content = std::string("invalid arguments for '") + name + "': " + error.what(), .error_code = std::make_error_code(std::errc::invalid_argument) };
  }
  catch (...) {
    runtime_.audit_tool_rejection(name, "unknown tool invocation failure");
    return { .content = "tool invocation failed with an unknown exception", .error_code = std::make_error_code(std::errc::io_error) };
  }
}
