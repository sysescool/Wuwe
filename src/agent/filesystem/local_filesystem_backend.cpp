...
filesystem_result failure_with_progress(
  ...
  const auto partial = result.affected_items > 0 || result.bytes_processed > 0 ||
    (destination_created != result.metadata.end() &&
     destination_created->second == "true") ||
    (parents_created != result.metadata.end() &&
     parents_created->second == "true");
...
bool path_within(
  ...
  for (auto root_it = root.begin(); root_it != root.end();
    ...
    if (candidate_it == candidate.end() ||
        !path_component_equal(*candidate_it, *root_it)) {
      return false;
    }
  }
  return true;
...
bool existing_path_contains_symlink(
  ...
  if (error || !current_exists) return false;
...
filesystem_result read_regular_file(
  const std::filesystem::path& path,
  ...
  if (error) return failure(status_from_error(error), path, error.message());
  ...
  if (!input) return failure(filesystem_status::io_error, path, "failed to open file");
  std::string content;
...
filesystem_result create_temporary_file(
  ...
  for (std::size_t attempt = 0; attempt < max_attempts; ++attempt) {
    ...
    if (handle == INVALID_HANDLE_VALUE) {
      ...
      return failure(status_from_error(error), destination,
    ...
    while (offset < content.size()) {
      ...
      const auto count = static_cast<DWORD>((std::min<std::size_t>)(
        content.size() - offset, 1024U * 1024U));
      if (!WriteFile(handle, content.data() + offset, count, &written, nullptr) ||
          written == 0) {
        write_error = std::error_code(
    ...
    return failure(filesystem_status::io_error, destination,
...
filesystem_result write_file_atomically(
  ...
  if (!valid_utf8(request.content)) {
    return failure(filesystem_status::type_mismatch, request.path,
  ...
  if (error) return failure(status_from_error(error), request.path, error.message());
  ...
  if (exists) {
    ...
    if (std::filesystem::is_symlink(existing_status)) {
      return failure(filesystem_status::permission_denied, request.path,
  ...
  if (request.expected_revision) {
    if (!exists) {
      return failure(filesystem_status::conflict, request.path,
  ...
  if (request.create_parent_directories && !request.path.parent_path().empty()) {
    const auto parent_existed =
      std::filesystem::exists(request.path.parent_path(), error);
    if (error) return failure(status_from_error(error), request.path, error.message());
  ...
  std::filesystem::path temporary;
  auto prepared = create_temporary_file(
    request.path, request.content, stop_token, temporary);
  if (!prepared.successful()) return with_parent_progress(std::move(prepared));
  struct cleanup {
  ...
  if (exists) {
    std::filesystem::permissions(
      temporary, existing_status.permissions(),
      std::filesystem::perm_options::replace, error);
    if (error) {
      return with_parent_progress(failure(
        status_from_error(error), request.path,
        "failed to preserve destination permissions: " + error.message()));
    }
  }
  if (stop_token.stop_requested()) {
    return with_parent_progress(failure(
      filesystem_status::cancelled, request.path, "operation cancelled"));
  }
  if (request.disposition == write_disposition::create_new) {
    if (!atomic_create(temporary, request.path, error)) {
      return with_parent_progress(failure(
        status_from_error(error), request.path, error.message()));
    }
  }
...
filesystem_entry_type entry_type_for(const std::filesystem::file_status& status) {
  if (std::filesystem::is_symlink(status)) return filesystem_entry_type::symlink;
  if (std::filesystem::is_regular_file(status)) return filesystem_entry_type::regular_file;
  if (std::filesystem::is_directory(status)) return filesystem_entry_type::directory;
  ...
  if (std::filesystem::is_regular_file(status)) {
    ...
    if (request.include_revision) {
      auto file = read_regular_file(
        request.path, request.max_revision_bytes, stop_token, false);
      if (!file.successful()) return file;
...
filesystem_result local_filesystem_backend::replace_text(
  ...
  if (request.old_text.empty()) {
    return failure(filesystem_status::invalid_request, request.path,
      "old_text must not be empty");
  }
  auto current = read_regular_file(
    request.path, request.max_result_bytes, stop_token);
  if (!current.successful()) return current;
  ...
  for (std::size_t pos = 0; (pos = current.content.find(request.old_text, pos)) != std::string::npos;
  ...
  if (request.max_result_bytes > 0 && updated.size() > request.max_result_bytes) {
    return failure(filesystem_status::limit_exceeded, request.path,
...
filesystem_result local_filesystem_backend::list_directory(
  ...
  if (!std::filesystem::is_directory(request.path, error)) {
    return failure(error ? status_from_error(error) : filesystem_status::type_mismatch,
      request.path, error ? error.message() : "path is not a directory");
  }
  ...
  if (!request.recursive) {
    for (std::filesystem::directory_iterator iterator(request.path, options, error), end;
      ...
      if (stop_token.stop_requested()) return failure(filesystem_status::cancelled, request.path, "operation cancelled");
      if (result.entries.size() >= request.max_entries) { result.truncated = true; break; }
    ...
    for (std::filesystem::recursive_directory_iterator iterator(request.path, options, error), end;
         iterator != end && !error; iterator.increment(error)) {
      if (stop_token.stop_requested()) return failure(filesystem_status::cancelled, request.path, "operation cancelled");
      if (iterator.depth() >= static_cast<int>(request.max_depth) && iterator->is_directory(error)) {
...
filesystem_result local_filesystem_backend::glob(
  ...
  std::regex pattern;
  try { pattern = glob_regex(request.pattern); }
  ...
  if (!listed.successful()) return listed;
...
filesystem_result local_filesystem_backend::search_text(
  ...
  if (request.max_files == 0 || request.max_results == 0 ||
  ...
  if (!files.successful()) return files;
  filesystem_result result { .status = filesystem_status::ok, .path = request.path };
  ...
  for (const auto& entry : files.entries) {
    if (entry.type != filesystem_entry_type::regular_file) continue;
    if (stop_token.stop_requested()) return failure(filesystem_status::cancelled, request.path, "operation cancelled");
    if (request.max_file_bytes > 0 && entry.size > request.max_file_bytes) { ++skipped_large; continue; }
    ...
    if (file.status == filesystem_status::type_mismatch) { ++skipped_binary; continue; }
    if (!file.successful()) { ++skipped_errors; continue; }
  ...
  result.metadata["file_enumeration_truncated"] =
...
filesystem_result local_filesystem_backend::create_directory(
  ...
  if (error) return failure(status_from_error(error), request.path, error.message());
  const auto created = request.recursive
    ? std::filesystem::create_directories(request.path, error)
...
filesystem_result local_filesystem_backend::copy_path(
  ...
  if (error) return failure(status_from_error(error), request.source, error.message());
  if (!std::filesystem::exists(source_status)) {
    return failure(filesystem_status::not_found, request.source,
      "source path does not exist");
  }
  if (std::filesystem::is_symlink(source_status)) {
    return failure(filesystem_status::permission_denied, request.source,
      "copying symbolic links is not supported");
  }
  const auto destination_exists = std::filesystem::exists(request.destination, error);
  if (error) {
    return failure(status_from_error(error), request.destination,
  ...
  if (existing_path_contains_symlink(
        ...
        offending_symlink, error)) {
    return failure(filesystem_status::permission_denied, offending_symlink,
  ...
  if (destination_exists) {
    const auto normalized_source =
      normalized_for_comparison(request.source, error);
    if (error) return failure(status_from_error(error), request.source, error.message());
    const auto normalized_destination =
      normalized_for_comparison(request.destination, error);
    if (error) {
    ...
    if (path_within(normalized_source, normalized_destination) &&
      ...
      return failure(filesystem_status::invalid_path, request.source,
        "source and destination must differ");
    }
  }
  if (destination_exists && !request.overwrite) {
    return failure(filesystem_status::already_exists, request.destination, "destination already exists");
  }
  filesystem_result result { .status = filesystem_status::ok, .path = request.source, .destination = request.destination };
  if (std::filesystem::is_regular_file(source_status)) {
    ...
    if (request.max_bytes > 0 && size > request.max_bytes) {
      return failure(filesystem_status::limit_exceeded, request.source,
        "copy exceeds max_bytes");
    }
    if (!request.destination.parent_path().empty()) {
      const auto parents_existed = std::filesystem::exists(
        request.destination.parent_path(), error);
      if (error) {
        return failure(status_from_error(error), request.destination,
          error.message());
      }
      const auto parents_created = std::filesystem::create_directories(
        request.destination.parent_path(), error);
      if (parents_created || (error && !parents_existed)) {
        result.metadata["parent_directories_created"] = "true";
      }
      if (error) {
        return failure_with_progress(std::move(result), status_from_error(error),
          request.destination, error.message());
      }
    }
    ...
    const auto copied = std::filesystem::copy_file(
      request.source, request.destination, options, error);
    if (!copied) {
      if (error) {
        return failure_with_progress(std::move(result), status_from_error(error),
          request.destination, error.message());
      ...
      return failure_with_progress(std::move(result),
        filesystem_status::already_exists, request.destination,
  ...
  if (!std::filesystem::is_directory(source_status) || !request.recursive) {
    return failure(filesystem_status::type_mismatch, request.source,
  ...
  const auto normalized_destination =
    normalized_for_comparison(request.destination, error);
  if (error) return failure(status_from_error(error), request.destination,
    "failed to normalize copy destination: " + error.message());
  if (path_within(normalized_destination, normalized_source)) {
    return failure(filesystem_status::invalid_path, request.destination,
  ...
  for (std::filesystem::recursive_directory_iterator iterator(request.source, options, error), end;
       iterator != end && !error; iterator.increment(error)) {
    if (stop_token.stop_requested()) {
      return failure(filesystem_status::cancelled, request.source,
        "operation cancelled");
    }
    ...
    if (error) break;
    if (std::filesystem::is_symlink(status)) {
      return failure(filesystem_status::permission_denied, iterator->path(), "copying symbolic links is not supported");
    }
    if (std::filesystem::is_directory(status)) {
      ...
      if (request.max_bytes > 0 &&
        ...
        return failure(filesystem_status::limit_exceeded, iterator->path(),
          "copy exceeds max_bytes");
      }
      ...
      return failure(filesystem_status::type_mismatch, iterator->path(),
  ...
  for (const auto& entry : plan) {
    ...
    if (existing_path_contains_symlink(
          request.destination, entry.destination,
          offending_symlink, error)) {
      return failure(filesystem_status::permission_denied, offending_symlink,
        "copy destination contains a symbolic link");
    }
  ...
  if (error) {
    if (!destination_exists) result.metadata["destination_created"] = "true";
    return failure_with_progress(std::move(result), status_from_error(error),
      request.destination, error.message());
  }
  if (destination_created) result.metadata["destination_created"] = "true";
  for (const auto& entry : plan) {
    if (stop_token.stop_requested()) {
      return failure_with_progress(std::move(result), filesystem_status::cancelled,
        entry.source, "operation cancelled during copy");
    }
    offending_symlink.clear();
    if (existing_path_contains_symlink(
          request.destination, entry.destination,
          offending_symlink, error)) {
      return failure_with_progress(std::move(result),
        filesystem_status::permission_denied, offending_symlink,
        "copy destination contains a symbolic link");
    }
    if (error) {
      return failure_with_progress(std::move(result), status_from_error(error),
        entry.destination, error.message());
    }
    if (entry.type == filesystem_entry_type::directory) {
      ...
      if (!error) result.bytes_processed += entry.size;
    }
    if (error) {
      return failure_with_progress(std::move(result), status_from_error(error),
        entry.destination, error.message());
...
filesystem_result local_filesystem_backend::move_path(
  ...
  std::stop_token stop_token) {
  std::scoped_lock lock(mutation_mutex_);
  if (stop_token.stop_requested()) return failure(filesystem_status::cancelled, request.source, "operation cancelled");
  std::error_code error;
  filesystem_result result {
    .status = filesystem_status::ok,
    .path = request.source,
  ...
  if (destination_exists) {
    const auto destination_status =
      std::filesystem::symlink_status(request.destination, error);
    if (error) {
      return failure(status_from_error(error), request.destination, error.message());
    }
    if (!std::filesystem::is_regular_file(source_status) ||
        !std::filesystem::is_regular_file(destination_status)) {
      return failure(filesystem_status::type_mismatch, request.destination,
...
filesystem_result local_filesystem_backend::remove_path(
  ...
  const auto status = std::filesystem::symlink_status(request.path, error);
  if (error) return failure(status_from_error(error), request.path, error.message());
  if (!std::filesystem::exists(status)) return failure(filesystem_status::not_found, request.path, "path does not exist");
  if (request.expected_revision) {
    auto current = read_regular_file(request.path, 0, stop_token);
    if (!current.successful()) return current;
    if (current.revision != *request.expected_revision) return failure(filesystem_status::conflict, request.path, "file revision does not match expected_revision");
  }
  filesystem_result result { .status = filesystem_status::ok, .path = request.path };
  if (!std::filesystem::is_directory(status) || std::filesystem::is_symlink(status)) {
    if (!std::filesystem::remove(request.path, error) || error) return failure(status_from_error(error), request.path, error.message());
    result.affected_items = 1;
    return result;
  }
  if (!request.recursive) {
    if (!std::filesystem::remove(request.path, error) || error) return failure(status_from_error(error), request.path, error ? error.message() : "directory is not empty");
    result.affected_items = 1;
    return result;
  }
  ...
  for (std::filesystem::recursive_directory_iterator iterator(request.path, options, error), end;
       iterator != end && !error; iterator.increment(error)) {
    if (stop_token.stop_requested()) return failure(filesystem_status::cancelled, request.path, "operation cancelled");
    if (paths.size() >= request.max_entries) return failure(filesystem_status::limit_exceeded, request.path, "remove entry limit exceeded");
  ...
  for (const auto& path : paths) {
    if (stop_token.stop_requested()) {
      return failure_with_progress(std::move(result), filesystem_status::cancelled,
        request.path, "operation cancelled during removal");
    }
    if (std::filesystem::remove(path, error)) ++result.affected_items;
    if (error) {
      return failure_with_progress(std::move(result), status_from_error(error),
        path, error.message());
    }
  }
  if (std::filesystem::remove(request.path, error)) ++result.affected_items;
  if (error) {
    return failure_with_progress(std::move(result), status_from_error(error),
