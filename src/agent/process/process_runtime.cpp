...
bool path_within(const std::filesystem::path& candidate, const std::filesystem::path& root) {
  ...
  for (auto root_it = root.begin(); root_it != root.end(); ++root_it, ++candidate_it) {
    if (candidate_it == candidate.end() || !same_path(*candidate_it, *root_it))
      return false;
  }
  return true;
}
...
bool valid_environment_entry(std::string_view key, std::string_view value = {}) {
  return !key.empty() && key.find('=') == std::string_view::npos &&
         key.find('\0') == std::string_view::npos && value.find('\0') == std::string_view::npos;
}

bool same_environment_key(std::string_view left, std::string_view right) {
#ifdef _WIN32
...
bool environment_override_allowed(const std::set<std::string>& allowed, std::string_view key) {
...
void set_environment_value(
  std::map<std::string, std::string>& environment, std::string key, std::string value) {
  const auto existing = std::find_if(environment.begin(),
    environment.end(),
    [&](const auto& entry) { return same_environment_key(entry.first, key); });
...
process_runtime::process_runtime(std::unique_ptr<process_backend> backend, process_policy policy,
  audit::audit_sink* audit, approval::approval_service* approvals)
    : backend_(std::move(backend)), policy_(std::move(policy)), audit_(audit),
      approvals_(approvals) {
  if (!backend_)
    throw std::invalid_argument("process_runtime requires a backend");
  ...
  if (policy_.default_workdir.empty())
  ...
  policy_.default_workdir = std::filesystem::weakly_canonical(default_workdir_input, error);
  ...
  if (policy_.shell_executable.empty())
    policy_.shell_executable = platform_shell();
  if (policy_.max_limits.max_argument_count == 0 || policy_.max_limits.max_argument_bytes == 0 ||
      policy_.max_limits.max_environment_count == 0 ||
      policy_.max_limits.max_environment_bytes == 0 || policy_.max_limits.max_stdin_bytes == 0 ||
      policy_.max_limits.max_stdout_bytes == 0 || policy_.max_limits.max_stderr_bytes == 0 ||
  ...
  for (const auto& [key, value] : policy_.base_environment) {
    ...
    if (std::any_of(environment_keys.begin(), environment_keys.end(), [&](const auto& existing) {
          return same_environment_key(existing, key);
        })) {
      throw std::invalid_argument("base_environment contains duplicate platform-equivalent keys");
    }
    environment_keys.push_back(key);
    environment_bytes =
      saturated_add(environment_bytes, saturated_add(key.size(), saturated_add(value.size(), 2)));
  }
  ...
  for (const auto& key : policy_.allowed_environment_overrides) {
    if (!valid_environment_entry(key)) {
      throw std::invalid_argument("allowed_environment_overrides contains an invalid key");
    }
    if (std::any_of(environment_keys.begin(), environment_keys.end(), [&](const auto& existing) {
          return same_environment_key(existing, key);
        })) {
  ...
  for (auto& executable : policy_.allowed_executables) {
    ...
    if (executable.has_parent_path()) {
      if (!executable.is_absolute()) {
        throw std::invalid_argument("allowed executable paths must be absolute or a bare filename");
      }
    ...
    else if (executable == "." || executable == ".." || executable.has_root_path()) {
      throw std::invalid_argument("bare allowed executable names must name a file");
    }
  }
  for (auto& search_path : policy_.executable_search_paths) {
    search_path = std::filesystem::weakly_canonical(search_path, error);
    if (error || !search_path.is_absolute() || !std::filesystem::is_directory(search_path, error)) {
      throw std::invalid_argument("executable search paths must be existing absolute directories");
    }
  }
  if (policy_.allow_shell) {
    policy_.shell_executable = std::filesystem::weakly_canonical(policy_.shell_executable, error);
...
process_result process_runtime::run(process_request request, std::stop_token stop_token) {
  return run_impl(std::move(request), false, stop_token);
}

process_result process_runtime::run_shell(shell_request request, std::stop_token stop_token) {
...
process_result process_runtime::run_impl(
  ...
  const auto denied = [&](std::string message,
                        process_termination_reason reason =
                          process_termination_reason::policy_denied) {
  ...
  if (shell && !policy_.allow_shell)
    return denied("shell execution is disabled");
  ...
  request.limits.max_stdout_bytes =
    normalize_limit(request.limits.max_stdout_bytes, policy_.max_limits.max_stdout_bytes);
  request.limits.max_stderr_bytes =
    normalize_limit(request.limits.max_stderr_bytes, policy_.max_limits.max_stderr_bytes);
  request.limits.max_stdin_bytes =
    normalize_limit(request.limits.max_stdin_bytes, policy_.max_limits.max_stdin_bytes);
  request.limits.max_argument_bytes =
    normalize_limit(request.limits.max_argument_bytes, policy_.max_limits.max_argument_bytes);
  request.limits.max_argument_count =
    normalize_limit(request.limits.max_argument_count, policy_.max_limits.max_argument_count);
  request.limits.max_environment_bytes =
    normalize_limit(request.limits.max_environment_bytes, policy_.max_limits.max_environment_bytes);
  request.limits.max_environment_count =
    normalize_limit(request.limits.max_environment_count, policy_.max_limits.max_environment_count);
  ...
  for (const auto& argument : request.arguments) {
    if (argument.find('\0') != std::string::npos)
      return denied("arguments must not contain NUL bytes");
  ...
  auto workdir = std::filesystem::weakly_canonical(
    workdir_input.is_absolute() ? workdir_input : policy_.working_directory_root / workdir_input,
    error);
  ...
  if (shell) {
    ...
    const auto requested_bare =
      !request.executable.is_absolute() && !request.executable.has_parent_path();
    ...
    if (request.executable.is_absolute() || request.executable.has_parent_path()) {
      ...
      const auto basename_allowed = std::any_of(policy_.allowed_executables.begin(),
        policy_.allowed_executables.end(),
        [&](const auto& allowed) {
      ...
      if (basename_allowed) {
        for (const auto& search_path : policy_.executable_search_paths) {
          candidates.push_back(
            std::filesystem::weakly_canonical(search_path / request.executable, error));
          error.clear();
        }
      }
    }
    for (const auto& candidate : candidates) {
      ...
      const auto allowed = std::any_of(policy_.allowed_executables.begin(),
        policy_.allowed_executables.end(),
    ...
    if (executable.empty())
      return denied("executable is not in the configured allowlist");
  }
  ...
  for (const auto& [key, value] : environment) {
    effective_environment_bytes = saturated_add(
      effective_environment_bytes, saturated_add(key.size(), saturated_add(value.size(), 2)));
  }
  ...
  capability::capability_request capability_request {
    ...
    .risk =
      shell ? capability::capability_risk_level::critical : capability::capability_risk_level::high,
  ...
  const auto approval_required =
    shell ? policy_.require_approval_for_shell : policy_.require_approval_for_process;
  if (approval_required) {
    if (!approvals_)
      return denied("approval required but no approval service is configured",
    ...
    try {
      ...
      return denied(std::string("approval service failed: ") + exception.what(),
        process_termination_reason::approval_denied);
    }
    catch (...) {
      return denied("approval service failed with an unknown exception",
        process_termination_reason::approval_denied);
    }
  ...
  process_result result;
  try {
    result = backend_->run(request, stop_token);
  }
  catch (const std::exception& exception) {
    ...
    result = { .termination_reason = process_termination_reason::backend_error,
      .error_message = "process backend threw an unknown exception" };
  }
  ...
  if (result.exit_code)
    finished.attributes["exit_code"] = std::to_string(*result.exit_code);
  if (!result.error_message.empty())
    finished.attributes["error"] = result.error_message;
  safely_publish(audit_, finished);
  return result;
}

const process_policy& process_runtime::policy() const noexcept {
  return policy_;
}
const process_backend* process_runtime::backend() const noexcept {
  return backend_.get();
}

void process_runtime::audit_tool_rejection(const std::string& tool_name, const std::string& reason,
