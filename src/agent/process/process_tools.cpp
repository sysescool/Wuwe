...
nlohmann::json limits_properties(const process_policy& policy) {
  return {
    { "timeout_ms",
      { { "type", "integer" },
        { "minimum", 1 },
        { "maximum", policy.max_limits.timeout.count() } } },
    { "max_stdout_bytes",
      { { "type", "integer" },
        { "minimum", 1 },
        { "maximum", policy.max_limits.max_stdout_bytes } } },
    { "max_stderr_bytes",
      { { "type", "integer" },
        { "minimum", 1 },
        { "maximum", policy.max_limits.max_stderr_bytes } } },
  };
}
...
nlohmann::json result_json(const process_result& result) {
  ...
  output["exit_code"] =
    result.exit_code ? nlohmann::json(*result.exit_code) : nlohmann::json(nullptr);
  output["process_id"] =
    result.process_id ? nlohmann::json(*result.process_id) : nlohmann::json(nullptr);
...
std::size_t parsed_size(const nlohmann::json& value, std::string_view name, std::size_t fallback) {
...
process_limits parse_limits(const nlohmann::json& args, const process_policy& policy) {
...
process_tool_provider::process_tool_provider(process_runtime& runtime, process_tool_options options)
...
std::vector<llm_tool> process_tool_provider::tools() const {
  ...
  if (!policy.allowed_executables.empty()) {
    ...
    result.push_back({
      ...
      .description = "Run one allowlisted executable with a structured argument vector, bounded "
                     "input/output, timeout, cancellation, approval, and audit.",
  ...
  if (options_.expose_shell_tool && policy.allow_shell) {
    ...
    result.push_back({
      ...
      .description = "Run a raw shell command. This is a high-risk, explicitly enabled capability "
                     "and normally requires approval.",
      .parameters_json_schema =
        nlohmann::json {
          { "type", "object" },
          { "properties", std::move(properties) },
